...
void HighscoresView::drawDifficultyNames()
{
	...
	{
		setCursoreAtPosition(getConsoleWidth() / vec.size() * i + 23 - i * 10, current.Y);
...
void HighscoresView::drawLines()
{
	...
	{
		setCursoreAtPosition(getConsoleWidth() / 2 - 2, current.Y + i);
...
void HighscoresView::clearTable()
{
	...
	for (size_t i = 0; i < 2; ++i)
	{
		...
		{
			std::string spaces(3 + getConsoleWidth() / 2 * (i + 1) - (3 + getConsoleWidth() / 3 * i) - 5, ' ');

			setCursoreAtPosition(3 + getConsoleWidth() / 2 * i, current.Y + (j * 2));
...
void HighscoresView::drawResults(Language languageToDraw)
{
	...
	for (size_t i = 0; i < 2; ++i)
	{
		...
		FilesManipulator().getHighscores(names, scores, static_cast<GameLogicComputer::Difficulty>(i));

		for (size_t j = 0; j < names.size(); ++j)
		{
			setCursoreAtPosition(-10 * i + 12 + getConsoleWidth() / 2 * i, current.Y + (j * 2));

			Printer() << names[j] << " ";

			std::string strToPrint = std::to_string(scores[j]);

			setCursoreAtPosition(-10 * i + getConsoleWidth() / 2 * (i + 1) - 4 - strToPrint.length(), current.Y + (j * 2));
			std::cout << strToPrint;
		}
	}
