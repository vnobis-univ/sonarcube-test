...
{
	GameLogicComputer::Difficulty difficulty = static_cast<GameLogicComputer::Difficulty>(getGameDifficulty());

	if (button == Button::LEFT)
	{
		if (difficulty == GameLogicComputer::Difficulty::easy)
			return;
		else
			setGameDifficulty(static_cast<GameLogicComputer::Difficulty> ((int)(difficulty)-1));
	}
	else if (button == Button::RIGHT)
	{
		if (difficulty == GameLogicComputer::Difficulty::hard)
			return;
		else
			setGameDifficulty(static_cast<GameLogicComputer::Difficulty> ((int)(difficulty)+1));
	}
