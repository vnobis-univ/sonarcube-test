...
bool OfflinePlayer::isHereStrikes(int i, int j)
{
...
void OfflinePlayer::enemyShipDestroyed(Ship ship)
{
	...
	case 0:
		for (int i = ship.head.x; i > ship.head.x - ship.length; --i)
		{
			placeTheStrike(i + 1, ship.head.y + 1);
			placeTheStrike(i - 1, ship.head.y - 1);
			placeTheStrike(i - 1, ship.head.y + 1);
			placeTheStrike(i + 1, ship.head.y - 1);
			placeTheStrike(i, ship.head.y + 1);
			placeTheStrike(i, ship.head.y - 1);
			placeTheStrike(i + 1, ship.head.y);
			placeTheStrike(i - 1, ship.head.y);
		}
		break;
	...
	case 1:
		for (int i = ship.head.y; i > ship.head.y - ship.length; --i)
		{
			placeTheStrike(ship.head.x + 1, i + 1);
			placeTheStrike(ship.head.x - 1, i - 1);
			placeTheStrike(ship.head.x + 1, i - 1);
			placeTheStrike(ship.head.x - 1, i + 1);
			placeTheStrike(ship.head.x + 1, i);
			placeTheStrike(ship.head.x - 1, i);
			placeTheStrike(ship.head.x, i + 1);
			placeTheStrike(ship.head.x, i - 1);
		}
		break;
	}
...
bool OnlinePlayer::isHereStrikes(int i, int j)
{
	if (i >= 0 && i < 10 && j >= 0 && j < 10)
	{
		return arrStrikes[j][i] == 1;
...
string OnlinePlayer::answer(Coordinate coord)
{
	...
	while (answer.length() == 2 || answer != "End")
	{
...
bool realPlayer::join(string id)
{
	...
	else
	{
		...
		connection.setComClient();
		connection.postMessage("Joined");
		if (connection.getMessage() != "Joined")
		{
			return false;
		}
		return true;
...
{
	if (!isHereStrikes(coord.x + 1, coord.y) && !player.isHereShips(coord.x + 1, coord.y)
		&& !isInVector(coords, Coordinate(coord.x + 1, coord.y)))
	{
		return Coordinate(coord.x + 1, coord.y);
	}
	else if (!isHereStrikes(coord.x - 1, coord.y) && !player.isHereShips(coord.x - 1, coord.y)
		&& !isInVector(coords, Coordinate(coord.x - 1, coord.y)))
	{
		return Coordinate(coord.x - 1, coord.y);
	}
	else if (!isHereStrikes(coord.x, coord.y + 1) && !player.isHereShips(coord.x, coord.y + 1)
	...
	if (coords.empty())
	{
		...
		if (player.isHereShips(coord.x, coord.y))
		{
			...
			case 0:
				for (int i = ship.head.x; i > ship.head.x - ship.length; --i)
				{
					if (!isHereStrikes(i, ship.head.y))
					{
			...
			case 1:
				for (int i = ship.head.y; i > ship.head.y - ship.length; --i)
				{
					if (!isHereStrikes(ship.head.x, i))
					{
